...
pub fn list(conn: &mut PooledConn, tournament_id: i64) -> mysql::Result<Vec<NamedItem>> {
    conn.exec_map(
        "SELECT id, name FROM categories WHERE tournament_id = ? ORDER BY id",
        (tournament_id,),
...
pub fn update(
    ...
    conn.exec_drop(
        "UPDATE categories SET name = ? WHERE id = ? AND tournament_id = ?",
...
pub fn delete(conn: &mut PooledConn, tournament_id: i64, id: i64) -> mysql::Result<usize> {
    ...
    tx.exec_drop(
        "DELETE FROM team_categories WHERE category_id = ? AND tournament_id = ?",
        (id, tournament_id),
    )?;
    tx.exec_drop(
        "DELETE FROM categories WHERE id = ? AND tournament_id = ?",
        (id, tournament_id),
    )?;
