...
pub fn list(conn: &mut PooledConn, tournament_id: i64) -> mysql::Result<Vec<NamedItem>> {
    conn.exec_map(
        "SELECT id, name FROM weight_classes WHERE tournament_id = ? ORDER BY id",
        (tournament_id,),
...
pub fn create(conn: &mut PooledConn, tournament_id: i64, name: &str) -> mysql::Result<i64> {
    conn.exec_drop(
        "INSERT INTO weight_classes (tournament_id, name) VALUES (?, ?)",
        (tournament_id, name),
    )?;
...
pub fn update(
    ...
    conn.exec_drop(
        "UPDATE weight_classes SET name = ? WHERE id = ? AND tournament_id = ?",
...
pub fn delete(conn: &mut PooledConn, tournament_id: i64, id: i64) -> mysql::Result<usize> {
    conn.exec_drop(
        "DELETE FROM weight_classes WHERE id = ? AND tournament_id = ?",
        (id, tournament_id),
    )?;
    Ok(conn.affected_rows() as usize)
}
...
pub fn get_by_id(conn: &mut PooledConn, tournament_id: i64, id: i64) -> mysql::Result<Option<NamedItem>> {
    let row: Option<(i64, String)> =
        conn.exec_first("SELECT id, name FROM weight_classes WHERE id = ? AND tournament_id = ?", (id, tournament_id))?;
...
pub fn get_by_name(
    ...
    let row: Option<(i64, String)> = conn.exec_first(
        "SELECT id, name FROM weight_classes WHERE tournament_id = ? AND LOWER(name) = LOWER(?)",
        (tournament_id, name),
    )?;
    Ok(row.map(|(id, name)| NamedItem { id, name }))
}
