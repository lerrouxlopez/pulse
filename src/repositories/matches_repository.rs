...
pub fn list(conn: &Connection, tournament_id: i64, scheduled_event_id: i64) -> rusqlite::Result<Vec<ScheduledMatch>> {
    let mut stmt = conn.prepare(
        "SELECT id, scheduled_event_id, mat, category, red, blue, status, location, match_time, round, slot, red_member_id, blue_member_id, is_bye
         FROM matches
...
pub fn create(
    ...
    conn.execute(
        "INSERT INTO matches (tournament_id, scheduled_event_id, mat, category, red, blue, status, location, match_time, round, slot, red_member_id, blue_member_id, is_bye)
    ...
    conn.execute(
        "UPDATE matches
         SET mat = ?1, category = ?2, red = ?3, blue = ?4, status = ?5, location = ?6, match_time = ?7,
             round = ?8, slot = ?9, red_member_id = ?10, blue_member_id = ?11, is_bye = ?12
         WHERE id = ?13 AND tournament_id = ?14 AND scheduled_event_id = ?15",
        params![
            mat,
            category,
            red,
