...
pub fn register_system_user(state: &State<AppState>, form: RegisterForm) -> Result<i64, AuthError> {
    ...
    let mut conn = db::open_conn(&state.pool).map_err(|err| AuthError::Storage(err.to_string()))?;
...
pub fn login_system_user(state: &State<AppState>, form: LoginForm) -> Result<i64, AuthError> {
    ...
    if let Some(user) = user {
        let parsed_hash =
            PasswordHash::new(&user.password_hash).map_err(|err| AuthError::Storage(err.to_string()))?;
        if Argon2::default()
